...
const ProductDetailScreen = () => {
  ...
  const [selectedZoomImage, setSelectedZoomImage] = useState<any>('');
  ...
  // Hàm lấy source ảnh phù hợp (require hoặc uri)
  const getImageSource = (img: any) => {
    ...
    if (typeof img === 'string') {
      ...
      try {
        const result = getImageRequire(img);
        return result;
      } catch (error) {
        return require('../../../../assets/icon.png');
      }
  ...
  const productImages = [
    getImageSource(product.imageDefault || product.image),
    getImageSource(product.imageDefault || product.image),
    getImageSource(product.imageDefault || product.image),
  ];
  ...
  const suggestedProducts = (searchProducts as any[])
    .filter(
      (item) =>
        item.collections &&
